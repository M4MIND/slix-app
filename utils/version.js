...
let version = file.match(new RegExp('"version": "[\\d]+.[\\d]+.[\\d]+"', "g"))[0];

version = version.match(new RegExp("[\\d]+.[\\d]+.[\\d]+", 'g'))[0].split('.');

version[2] = Number(version[2]) + 1;

version = `"version": "${version.join('.')}"`;

file = file.replace(new RegExp('"version": "[\\d]+.[\\d]+.[\\d]+"', 'g'), version);
